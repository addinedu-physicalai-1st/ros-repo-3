    ...
    private:
        void timer_callback()
        {
            ...
            for(int i = 0; i < 5; i++)
            {
                ...
                // I2C Write/Read with minimal delay. 
                // Using bitmasking for 12-bit ADC result assembly.
                wiringPiI2CRawWrite(fd_, &registers[i], 1);
                delayMicroseconds(6000); // 6ms delay using hardware-friendly delay

                wiringPiI2CRawRead(fd_, data, 2);
                // Bitmasking: Assembly of 12-bit result from two 8-bit registers
                // High byte (data[0]) contains bits 11-4, Low byte (data[1]) contains bits 3-0 in its upper nibble

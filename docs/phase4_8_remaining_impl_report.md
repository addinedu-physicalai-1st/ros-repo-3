...
- [ ] DynamixelMotor 실 하드웨어 검증 (JointState.position 라디안 값)
- [ ] SllidarDriver RPLiDAR SDK 런타임 검증
- [ ] WS2811 LED/Lamp 런타임 검증
- [ ] ILI9341 LCD + EmotionRenderer 실물 표시 검증
- [ ] BNO055 IMU dps→rad/s 변환 결과 검증

### 기능 확장 (향후)
- [ ] Nav2 브리지 (NavWorker) — ROS2 nav2_msgs 브릿지
- [ ] rl_config.yaml 별도 파싱 (현재 robot_config.yaml만 지원)
- [ ] station_config.yaml Python 측 파싱
- [ ] GUI 위젯 단위 테스트 (PyQt6 mock)
- [ ] TCP/UDP 루프백 실제 통합 테스트 (C++ 서버 + Python 클라이언트)
...
| Phase 8 (통합 테스트) | 🔶 부분 | PC 단위 테스트 통과, 실기 통합 테스트 미진행 |
